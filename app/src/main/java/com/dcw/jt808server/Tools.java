...
public class Tools {
    ...
    public static byte[] byteTobyte(byte[] src, int startIndex, int length) {
        ...
        int i = 0;
        for (int j = startIndex; i < length && j<src.length; ++j) {
            des[i] = src[j];
            ++i;
        }
        return des;
    ...
    public static String yxReversaltoString(byte[] bytes) {
        String str = parseByte2HexStr(bytes).toUpperCase();
        str = str.replaceAll("7D02", "7E").replaceAll("7D01", "7D");
        return str;
    }
     ...
     * @return
     */
    public static String byteToStringforUTF(byte[] src, int startIndex, int length) {
        byte[] des = new byte[length];
        int i = 0;
        for (int j = startIndex; i < length; ++j) {
            des[i] = src[j];
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Tools.stringToByte(Tools.appendRight("123456789", 16)).length);
        String s = "粤B12345";// 字符串
     ...
     * @param bytes
     */
    public static void decoderQH7FData(byte[] bytes) {
        try {
            int nOff = 2;
            //验证校验码
            byte[] jymdata = Tools.byteTobyte(bytes, nOff, bytes.length - 3);
            nOff += bytes.length - 3;
            byte[] jym = Tools.byteTobyte(bytes, nOff, 1);

            byte[] data = Tools.byteTobyte(bytes, 14, bytes.length - 16);
            byte[] msgid = Tools.byteTobyte(bytes, 12, 2);
            byte[] seqbyte = Tools.byteTobyte(bytes, 10, 2);
            int sqe = Integer.parseInt(Tools.parseByte2HexStr(seqbyte), 16);

            int nMsgId = Integer.parseInt(Tools.parseByte2HexStr(msgid), 16);
